...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-4xl" x-data="quizPlayer()" x-init="init()">
        ...
        <!-- Quiz Card -->
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden relative transition-all"
                ...
                <div class="p-8 sm:p-12 flex flex-col h-full">
                    ...
                    <div class="flex justify-between items-start mb-10">
                        <h2 class="text-gray-500 font-semibold text-[15px]">
                            ...
                            <span x-show="currentQuestion?.type === 'pronunciation'">Which is the correct
                                pronunciation?</span>
                            <span x-show="currentQuestion?.type === 'voice'">Read the article below and record your
                                voice</span>
                        </h2>
                        <div
                            class="bg-[#f1f5f9] text-[#64748b] text-[13px] font-bold px-4 py-1.5 rounded-full flex-shrink-0">
                    ...
                    <div class="flex-1 flex flex-col justify-center">
                        ...
                        <template x-if="currentQuestion?.type === 'letter'">
                            <div class="w-full">
                                ...
                                <div
                                    class="bg-[#eefcf2] border border-[#d1f4e0] rounded-3xl w-40 h-40 flex items-center justify-center mx-auto mb-10">
                                    <span class="font-arabic text-6xl text-[#0f5132] leading-none"
                                        x-text="currentQuestion.question_text"></span>
                                </div>

                                <!-- 2x2 Options Grid -->
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
                                    <template x-for="opt in currentQuestion.options" :key="opt.id">
                                        <button @click="selectOption(opt.id)" :disabled="answered"
                                            ...
                                            <!-- Correct Checkmark -->
                                            <svg x-show="answered && opt.id === correctOptionId"
                                                class="absolute right-5 w-5 h-5 text-[#10b981]" fill="none"
                        ...
                        <template x-if="currentQuestion?.type === 'pronunciation'">
                            <div class="w-full">
                                ...
                                <div
                                    class="bg-[#eefcf2] border border-[#d1f4e0] rounded-3xl w-full py-12 flex items-center justify-center mb-10 max-w-3xl mx-auto">
                                    <span class="font-arabic text-[5rem] text-[#0f5132] leading-none"
                                        x-text="currentQuestion.question_text"></span>
                                </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl mx-auto">
                                    <template x-for="opt in currentQuestion.options" :key="opt.id">
                                        <button @click="selectOption(opt.id)" :disabled="answered"
                                            ...
                                            <!-- Correct Checkmark -->
                                            <svg x-show="answered && opt.id === correctOptionId"
                                                class="w-5 h-5 text-[#10b981] ml-3 flex-shrink-0" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M5 13l4 4L19 7" />
                                            </svg>
                        ...
                        <template x-if="currentQuestion?.type === 'voice'">
                            <div class="w-full flex flex-col items-center">
                                ...
                                <div
                                    class="bg-[#eefcf2] border border-[#d1f4e0] rounded-3xl w-full p-10 mb-8 max-w-4xl mx-auto text-center">
                                    <p class="font-arabic text-4xl text-[#0f5132] leading-[2]"
                                        x-text="currentQuestion.question_text" dir="rtl"></p>
                                </div>

                                <!-- Instructions -->
                                <p class="text-[#94a3b8] text-[15px] font-medium mb-8">Record your voice (Max 1 minute)
                                </p>

                                <!-- Voice Recorder UI -->
                                <div class="flex items-center justify-center gap-6 mb-5 w-full"
                                    ...
                                    <div class="flex items-center gap-2 h-10">
                                        <template x-for="h in [10, 16, 24, 14, 20, 12, 18, 22]">
                                            <div class="waveform-bar"
                                                :style="`--h: ${h}px; --d: ${0.3 + Math.random() * 0.5}s; animation-delay: ${Math.random() * 0.5}s; height: 4px`">
                                            </div>
                                        </template>
                                    </div>
                                    ...
                                    <button @click="toggleRecording()"
                                        ...
                                        <!-- Mic icon -->
                                        <svg x-show="!isRecording" class="w-8 h-8" fill="currentColor"
                                            viewBox="0 0 24 24">
                                            <path
                                ...
                                <p class="text-[#cbd5e1] text-[13px] font-medium" x-show="!audioBlob && !isRecording">
                                    Click the microphone to start recording</p>

                                <!-- Audio Preview & Reset -->
                                <div x-show="audioBlob && !isRecording"
                                    class="mt-6 flex flex-col items-center gap-4 w-full max-w-sm">
                                    <audio :src="audioUrl" controls class="w-full h-10 rounded-full bg-gray-50"></audio>
                                    <button @click="resetRecording()"
                                        class="text-sm text-[#94a3b8] hover:text-red-500 font-medium underline underline-offset-2">Record
                                        Again</button>
                                </div>

                                <p class="text-red-500 text-sm mt-4 font-medium" x-show="voiceError"
                                    x-text="voiceError"></p>
                    ...
                    <div class="mt-12 flex items-center justify-between">
                        ...
                        <!-- Right: Next / Submit -->
                        <div>
                            <button x-show="answered && currentQuestion?.type !== 'voice'" @click="nextQuestion()"
                            ...
                            <button x-show="currentQuestion?.type === 'voice' && audioBlob && !isRecording"
                                @click="submitVoice()" :disabled="uploading"
            ...
            <template x-if="quizDone">
                <div class="p-16 flex flex-col items-center justify-center h-full text-center min-h-[500px]">
                    <div class="w-20 h-20 bg-[#eafbf1] rounded-full flex items-center justify-center mb-6">
                        <svg class="w-10 h-10 text-[#10b981] animate-bounce" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
