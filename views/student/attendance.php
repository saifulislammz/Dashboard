...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-gray-50">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <!-- Summary Card -->
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 <?= $bgColor ?> border-l-4 border-l-<?= str_contains($bgColor, 'green') ? 'green' : (str_contains($bgColor, 'yellow') ? 'yellow' : 'red') ?>-400">
            <div class="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div class="flex-1 grid grid-cols-3 gap-4 text-center">
                    ...
                    <div class="bg-green-50 rounded-xl p-4 shadow-sm">
                        ...
                        <p class="text-xs text-green-500 mt-1 uppercase tracking-wide font-medium">Present</p>
                    </div>
                    <div class="bg-red-50 rounded-xl p-4 shadow-sm">
                        <p class="text-3xl font-bold text-red-600"><?= $absent ?></p>
                        <p class="text-xs text-red-400 mt-1 uppercase tracking-wide font-medium">Absent</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <p class="text-xs text-gray-500">
                    ...
                    ?>
                    <?php if ($needed > 0): ?>
                        ⚠️ You need to attend <strong><?= $needed ?></strong> more consecutive session<?= $needed > 1 ? 's' : '' ?> to reach 75%.
                    <?php else: ?>
                        ✅ You have reached the required 75% attendance.
                    <?php endif; ?>
                </p>
            </div>
            <?php endif; ?>
                        ...
                        <div class="flex-shrink-0">
                            <?php if ($isPresent): ?>
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">
                                    Present
                                    <?php if (!empty($s['join_time'])): ?>
                                        <span class="ml-1.5 text-green-500 font-normal">
                                            <?= htmlspecialchars(date('h:i A', strtotime($s['join_time'])), ENT_QUOTES, 'UTF-8') ?>
                                        </span>
                                    <?php endif; ?>
                                </span>
                            <?php else: ?>
