        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold <?= $statusClasses ?>">
                        <?= ucfirst($status) ?>
                    </span>
                </div>
            ...
            <div class="flex items-center gap-3 ml-8 sm:ml-0">
                ...
                <form method="POST" action="/admin/invoices/update_status.php" class="flex items-center">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(generateCsrfToken(), ENT_QUOTES, 'UTF-8') ?>">
                    <input type="hidden" name="id" value="<?= (int) $invoice['id'] ?>">
                    <select name="status" class="px-4 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-colors shadow-sm cursor-pointer appearance-none pr-10" style="background-image: url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23475569%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E'); background-repeat: no-repeat; background-position: right 1rem top 50%; background-size: 0.65rem auto;" onchange="this.form.submit()">
            ...
            <div class="bg-gradient-to-r from-primary to-primary px-8 py-6 text-white">
                <div class="flex flex-col sm:flex-row justify-between gap-4">
                    <div>
                        ...
                        <?php if (!empty($settings['institution_tagline'])): ?>
                            <p class="text-white/70 text-sm mt-0.5">
                                <?= htmlspecialchars($settings['institution_tagline'], ENT_QUOTES, 'UTF-8') ?></p>
                        <?php endif; ?>
                        <?php if (!empty($settings['institution_address'])): ?>
                            <p class="text-white/60 text-xs mt-1">
                                <?= htmlspecialchars($settings['institution_address'], ENT_QUOTES, 'UTF-8') ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="text-right">
                        ...
                        <span
                            class="inline-flex items-center mt-2 px-3 py-1 rounded-full text-xs font-bold <?= match ($status) { 'paid' => 'bg-green-400/20 text-green-200', 'unpaid' => 'bg-yellow-400/20 text-yellow-200', default => 'bg-white/20 text-white/80'} ?>">
                            <?= strtoupper($status) ?>
                        </span>
                    </div>
                </div>
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <?php if ($sEmail): ?>
                            <p class="text-sm text-[#64748b] mt-1"><?= $sEmail ?></p><?php endif; ?>
                        <?php if ($sPhone): ?>
                            <p class="text-sm text-[#64748b]"><?= $sPhone ?></p><?php endif; ?>
                        <?php if ($sCountry): ?>
                            <p class="text-sm text-[#64748b]"><?= $sCountry ?></p><?php endif; ?>
                    </div>
                    <!-- Invoice Details -->
